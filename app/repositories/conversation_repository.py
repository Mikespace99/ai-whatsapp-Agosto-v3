...
def get_or_create_conversation(tenant_id: str, customer_id: str):
    ...
    if response.data:
        ...
        if not _is_stale(existing):
            print(
                f"[TIMEOUT] Conversazione riutilizzata "
                f"{existing['conversation_id']} "
                f"(ultima attivita': {existing.get('last_message_at')})"
            )
            return existing
        ...
        print(
            f"[TIMEOUT] Conversazione {existing['conversation_id']} "
            f"scaduta per timeout (ultima attivita': "
            f"{existing.get('last_message_at')}), la chiudo e ne creo una nuova."
        )
    ...
    print(f"[TIMEOUT] Nuova conversazione creata: {conversation_id}")
...
def merge_booking_preferences(conversation_id: str, current: dict, new: dict):
    """
    Applica solo i campi NON nulli restituiti da AI#1 sopra le
    preferenze gia' persistite, senza cancellare cio' che era gia'
    noto (Regola 3 del Context Contract: nessuno cancella dati
    presenti senza motivo).
    """
    if not new:
        return current
    ...
    supabase.table("conversation_context").update({
        "booking_preferences": current
...
def try_claim_message(message_id: str) -> bool:
    """
    Deduplicazione a livello di message_id (Meta WhatsApp).

    Ritorna True se questo messaggio non era mai stato visto prima
    (quindi va elaborato), False se e' un duplicato/retry gia' gestito
    (quindi va ignorato senza rielaborare nulla).

    In caso di errore imprevisto (es. problema di rete verso Supabase,
    non legato a un vero duplicato) si fallisce "aperto": meglio
    rischiare un doppio invio raro che bloccare il bot per un errore
    infrastrutturale temporaneo.
    """
    if not message_id:
        # Nessun message_id disponibile: non possiamo deduplicare,
        # si procede (caso raro, es. messaggi senza id valido).
        return True
    ...
    try:
        ...
        if "duplicate" in error_text or "23505" in error_text:
            print(
                f"[INFO] Messaggio {message_id} gia' elaborato in "
                f"precedenza, ignorato (probabile retry di Meta)."
            )
            return False

        print(
            f"[WARNING] try_claim_message: errore inatteso, si procede "
            f"comunque per non bloccare il bot: {error}"
        )
        return True
