...
from app.web.landing.routes import router as landing_router
from app.web.auth.routes import router as auth_router
from app.web.onboarding.routes import router as onboarding_router
...
def process_message(message):
    ...
    intent_result = parse_intent(
        ...
        suspended=suspended,
        timezone_name=tenant.get("timezone") or "Europe/Rome"
    )
    ...
    new_preferences = intent_result.get("preferences") or {}
    if new_preferences:
        merged = merge_booking_preferences(
            conversation["conversation_id"],
            conversation_context.get("booking_preferences"),
            new_preferences
        )
        conversation_context["booking_preferences"] = merged
    ...
    if assistant_message and should_send:
        ...
        try:
            whatsapp_response = send_whatsapp_message(
                ...
                access_token=whatsapp_account.get("access_token"),
                phone_number_id=whatsapp_account.get("phone_number_id")
            )
